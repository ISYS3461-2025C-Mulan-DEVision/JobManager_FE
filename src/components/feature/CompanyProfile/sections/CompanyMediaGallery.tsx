    ...
    return (
      ...
      >
        ...
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <ConfirmButton onConfirm={handleDelete} variant="danger" size="sm" className="cursor-pointer">
            <Trash2 className="w-4 h-4" />
          </ConfirmButton>
        </div>

        {/* Drag indicator */}
        <div data-no-view className="absolute top-2 left-2 bg-white bg-opacity-80 rounded p-1 cursor-move">
...
export const CompanyMediaGallery: React.FC = () => {
  ...
  const handleContainerDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const reorderPayload: MediaReorderItem[] = newItems.map(
        (item, index) => ({
          mediaId: item.id,
          displayOrder: index,
        })
