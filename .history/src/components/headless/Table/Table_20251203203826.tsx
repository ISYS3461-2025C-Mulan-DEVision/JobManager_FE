...
interface HeadlessTableProps<T> {
  ...
  pageSize?: number;
  className?: string;
  renderHeader?: (columns: TableColumn<T>[], requestSort: (key: keyof T) => void, sortConfig: { key: keyof T; direction: 'asc' | 'desc' } | null) => React.ReactNode;
  renderRow?: (item: T, columns: TableColumn<T>[]) => React.ReactNode;
  renderPagination?: (currentPage: number, totalPages: number, goToPage: (page: number) => void) => React.ReactNode;
}
...
export const HeadlessTable = <T extends { id: string | number }>({
  ...
  return (
    <div className={className}>
      ...
      {renderPagination && renderPagination(currentPage, totalPages, goToPage)}
    </div>
  );
};
